...
COSXVisaAdapter::COSXVisaAdapter()
    : m_defaultRM(VI_NULL)
    , m_session(VI_NULL)
{
}
...
bool COSXVisaAdapter::Open(const std::string& address, int port, double timeout)
{
    (void)port;
...
bool COSXVisaAdapter::IsOpen() const
{
    return m_session != VI_NULL;
}

std::string COSXVisaAdapter::SendQuery(const std::string& command)
{
    SendWrite(command);
...
void COSXVisaAdapter::SendWrite(const std::string& command)
{
    if (m_session == VI_NULL)
        throw std::runtime_error("OSX VISA adapter 未连接。");
