...
CViaviPCT4AllTcpAdapter::CViaviPCT4AllTcpAdapter()
...
bool CViaviPCT4AllTcpAdapter::Open(const std::string& address, int port, double timeout)
{
...
void CViaviPCT4AllTcpAdapter::Close()
{
...
bool CViaviPCT4AllTcpAdapter::IsOpen() const
{
    return m_socket != INVALID_SOCKET;
}

void CViaviPCT4AllTcpAdapter::SetReadTimeout(DWORD timeoutMs)
{
    if (m_socket != INVALID_SOCKET)
    {
...
std::string CViaviPCT4AllTcpAdapter::SendQuery(const std::string& command)
{
    SendWrite(command);
...
void CViaviPCT4AllTcpAdapter::SendWrite(const std::string& command)
{
    if (m_socket == INVALID_SOCKET)
        throw std::runtime_error("Viavi PCT4All TCP adapter not connected.");

    std::string fullCmd = command + "\n";
    int sent = send(m_socket, fullCmd.c_str(), static_cast<int>(fullCmd.size()), 0);
    if (sent == SOCKET_ERROR)
        throw std::runtime_error(std::string("Viavi PCT4All TCP send failed: WSA ")
