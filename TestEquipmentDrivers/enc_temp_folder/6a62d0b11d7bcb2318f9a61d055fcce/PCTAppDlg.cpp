...
void CPCTAppDlg::OnBnClickedZeroing()
{
    ...
    m_editChFrom.GetWindowText(fromStr);
    m_editChTo.GetWindowText(toStr);
    int j1State = m_radioJ1.GetCheck();
    int j2State = m_radioJ2.GetCheck();
    int launchPort = (j2State == BST_CHECKED) ? 2 : 1;
    ...
    {
        char _db[256];
        sprintf_s(_db, "{\"j1State\":%d,\"j2State\":%d,\"launchPort\":%d}", j1State, j2State, launchPort);
        DbgLog("Zeroing:RadioRead", "J1/J2 radio state", "H1", _db);
    }
    // #endregion
         ...
         avgTime, chFrom, chTo, pStop]() -> WorkerResult*
    {
        ...
        try
        {
            ...
            {
                char launchResp[256] = {};
                pLoader->SendCommand(":PATH:LAUN?", launchResp, sizeof(launchResp));
                char _db[512]; std::string _sl = DbgSafe(launchResp);
                sprintf_s(_db, "{\"launchPortSent\":%d,\"launchActual\":\"%s\"}", launchPort, _sl.c_str());
                DbgLog("Zeroing:LaunchVerify", "PATH:LAUN? query-back", "H2", _db);
            }
            // #endregion
