...
CViaviPCTTcpAdapter::CViaviPCTTcpAdapter()
...
bool CViaviPCTTcpAdapter::Open(const std::string& address, int port, double timeout)
{
...
void CViaviPCTTcpAdapter::Close()
{
...
bool CViaviPCTTcpAdapter::IsOpen() const
{
    return m_socket != INVALID_SOCKET;
}

void CViaviPCTTcpAdapter::SetReadTimeout(DWORD timeoutMs)
{
    if (m_socket != INVALID_SOCKET)
    {
...
std::string CViaviPCTTcpAdapter::SendQuery(const std::string& command)
{
    SendWrite(command);
...
void CViaviPCTTcpAdapter::SendWrite(const std::string& command)
{
    if (m_socket == INVALID_SOCKET)
        throw std::runtime_error("Viavi PCT TCP adapter not connected.");

    std::string fullCmd = command + "\n";
    int sent = send(m_socket, fullCmd.c_str(), static_cast<int>(fullCmd.size()), 0);
    if (sent == SOCKET_ERROR)
        throw std::runtime_error(std::string("Viavi PCT TCP send failed: WSA ")
