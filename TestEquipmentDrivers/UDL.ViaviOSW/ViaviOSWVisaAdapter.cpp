...
CViaviOSWVisaAdapter::CViaviOSWVisaAdapter()
    : m_defaultRM(VI_NULL)
    , m_session(VI_NULL)
    , m_bufferSize(4096)
{
...
bool CViaviOSWVisaAdapter::Open(const std::string& address, int port, double timeout)
{
    (void)port;
...
void CViaviOSWVisaAdapter::Close()
{
...
bool CViaviOSWVisaAdapter::IsOpen() const
{
    return m_session != VI_NULL;
}

std::string CViaviOSWVisaAdapter::SendQuery(const std::string& command)
{
    SendWrite(command);
...
void CViaviOSWVisaAdapter::SendWrite(const std::string& command)
{
    if (m_session == VI_NULL)
        throw std::runtime_error("Viavi OSW VISA adapter 未连接。");
...
void CViaviOSWVisaAdapter::SetReadTimeout(DWORD timeoutMs)
{
