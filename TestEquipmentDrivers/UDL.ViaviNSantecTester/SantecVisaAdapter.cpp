...
bool CSantecVisaAdapter::Open(const std::string& address, int port, double timeout)
{
    (void)port;
...
bool CSantecVisaAdapter::IsOpen() const
{
    return m_session != VI_NULL;
}

std::string CSantecVisaAdapter::SendQuery(const std::string& command)
{
    SendWrite(command);
...
void CSantecVisaAdapter::SendWrite(const std::string& command)
{
    if (m_session == VI_NULL)
        throw std::runtime_error("Santec VISA adapter 未连接。");
...
void CSantecVisaAdapter::SetReadTimeout(DWORD timeoutMs)
{
